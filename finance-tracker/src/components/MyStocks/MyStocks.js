...
class MyStocks extends Component {
    ...
    createRows = () => {
        ...
        Object.keys(this.props.myStocks).map(row => {
            ...
            Axios.get('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol='+dataRow.symbol+'&outputsize=compact&apikey=B9740XFFGGSQWKQE')
                .then(response => {
    ...
    render() {
        if(Object.keys(this.props.myStocks).length > 4 && Object.keys(this.props.myStocks).length !== Object.keys(this.state.myStocks).length && this.state.isError === false) {
            this.createRows();
        }
